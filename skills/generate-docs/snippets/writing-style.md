...
- **Third person, no "we" or "you"** in reference docs: "Inspections transition through five states" — not "We move inspections through five states"
...
Heading text should be the natural name (`POST /api/v1/inspections/`, `InspectionService.create_inspection`, `Inspection`) — not a description ("Creating an inspection").

## Code references

Format: `path/to/file.ext:LINE`. Example: `core/inspections/services.py:42`.

When linking from markdown: `[InspectionService.create_inspection](../../core/inspections/services.py#L42)`.

When citing an endpoint by handler: include both the path and the class:
`Handler: ContractViewSet.cancel ([core/contracts/views/contract_viewset.py:L142](../../...))`.
...
- ❌ "This module is responsible for handling..." → "Owns inspection lifecycle"
...
- ✅ Specific names: not "the service", but "InspectionService"
- ✅ Specific numbers: not "many actions", but "12 actions"
- ✅ Specific reasons: not "for performance", but "to avoid N+1 by prefetching the related Client"
...
❌ Bad:
> This module provides various services for inspection management. There are several different operations that can be performed, including creating, updating, and deleting inspections. The system uses a robust state machine to ensure inspections progress through their lifecycle in a consistent manner.

✅ Good:
> Owns the inspection lifecycle from scheduling through result publication. Five public services: `create_inspection`, `assign_inspector`, `mark_in_progress`, `submit_results`, `cancel`. State transitions are enforced by `Inspection.status` (see [flows/status-machine.md](flows/status-machine.md)).

❌ Bad:
> The cancel action allows you to cancel a contract. It performs the necessary updates to the contract record and may also create related records as needed.
