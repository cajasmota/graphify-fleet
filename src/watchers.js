...
// Merge daemon was removed when the MCP server moved to a graphs-dir +
// per-file mtime-reload model. Stub functions retained as no-ops for any
// caller that hasn't been migrated yet (and so `gfleet update` can clean up
// the old launchd / systemd / scheduled-task entries via migration helpers
// in `src/integrations.js`).
export function mergeDaemonLabel(group) { return `ai.graphify.fleet.${group}.merge-daemon`; }
export function installMergeDaemon() { /* no-op: merge daemon retired */ }
export function uninstallMergeDaemon() { /* no-op: merge daemon retired (cleanup happens in integrations.migrateLegacyArtifacts) */ }
